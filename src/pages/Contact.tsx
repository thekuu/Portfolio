      ...
      <div className="container mx-auto px-6 md:px-12 max-w-4xl text-center">
        ...
        >
          <a 
            ...
            className="group flex items-center justify-center gap-4 text-xs sm:text-lg md:text-2xl font-mono tracking-[0.1em] md:tracking-[0.15em] uppercase text-muted-foreground md:hover:text-foreground transition-colors hoverable w-full sm:w-auto py-4 px-2 relative"
          >
            <span className="break-all text-center">zekaryasgeremew@gmail.com</span>
            <ArrowRight className="md:md:group-hover:translate-x-4 transition-transform shrink-0 w-5 h-5 md:w-8 md:h-8" />
            
            <span className="absolute bottom-0 left-0 w-full h-[1px] bg-border transition-colors"></span>
            <span className="absolute bottom-0 left-0 w-0 h-[2px] bg-foreground transition-all duration-500 md:group-hover:w-full"></span>
          </a>
...
function SocialLink({ href, icon, label }: { href: string, icon: React.ReactNode, label: string }) {
  return (
    <a 
      ...
      className="flex flex-col items-center gap-3 text-muted-foreground md:hover:text-foreground transition-colors hoverable"
    >
      <div className="w-12 h-12 rounded-full border border-border flex items-center justify-center bg-muted/50 md:hover:bg-muted transition-colors">
        {icon}
      </div>
